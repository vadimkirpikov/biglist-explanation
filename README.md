...
- **void Insert(int index, T item)** - вставляет элемент `item` по индексу `index`, алгоритмическая сложность - `O(log(N))`;
...
Большинство методов класса `Node` могут как модифицировать существующий узел, вернув `this`, могут вернуть новый узел.

### Обзор полей класса Node
У класса `Node` есть следующие поля:
- **bool volatile shared** - поле показывает, является ли узел разделенным, т. е. может ли он испоьзоваться несколькими коллекциями `BigList`. Поле является волатильным, что гарантирует, что все потоки будут работать с его актуальным значением.
- **int count** - кол-во элементов на данном узле, за его чтение отвечает свойство `Count`, которое переопределяется в производных от `Node` классах;
- **int depth**  - глубина дерева, корнем которого является данных узел (поддерева основного дерева). За его чтение отвечает свойство `Depth`, которое переопределяется в производных классах 

### Балансировка
После каждой операции изменения коллекции происходит ее проверка на сбалансированность. Условие для запуска балансировкки: глубина дерева больше чем `BALANCEFACTOR`, затем декрево должно иметь глубину, превышающую `MAXFIB` более чем на `BALANCEFACTOR` или кол-во элементов, меньшее заданного числа Фибоначчи для заданной глубины. Балансировка происходит следующим образом:
- у узла есть метод `IsBalanced()`, проверяющий его на сбалансированность;
- происходит рекурсивный проход по всем узлам дерева для поиска всех сбалансированнывх узлов с максимальным количесвтом (если некоторый `ConcatNode` сбалансирован, то дальше проход по нему не происходит, любой `LeafNode` всегда сбалансирован).
- найденные узлы добалвяются в массив `rebalancedArray` по индексу, зависящему от количества элементов на узле (не должно превосходить определенного числа Фибоначчи). Таким образом, сбалансированные узлы со схожим количесвтом будут добавляться в массив по одному и тому же индексу, если в массиве уже есть узел по заданному индексу, то происходит их соединение или слияние.
- после того как все узлы перебраны и результат выборки записан в массив, новые узлы заново соединяются и главный узел (который не является дочерним ни для одного из узлов) становится узлом `root`.


## Вывод
В общем и целом, краткий обзор коллекции `BigList<T>` закончен, рекомендуется использвать данную коллекцию тогда когда требуется редкое получение данных из коллекции, но частая вставка в середину коллекции. поскольку получение элемента в `BigList<T>`- логарифмическое, 
